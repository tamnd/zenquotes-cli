// Package cli assembles the zenquotes command tree from the zenquotes
...
// NewApp assembles the kit application from the zenquotes domain. The
...
//
// To add a command, declare it in zenquotes/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := zenquotes.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(zenquotes.Domain{}).Register(app)
